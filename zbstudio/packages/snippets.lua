...
local function OnEscCancel(self, editor, event)
  ...
  manager:cancel_current(editor)

  return true
end

local TAB_ACTIVATION_ENABLED
...
Package.onRegister = function(package)
  ...
  TAB_ACTIVATION_ENABLED = config.settings.tab_activation
end
...
Package.onEditorKeyDown = function(self, editor, event)
  if not OnEscCancel(self, editor, event) then
    return false
  end

  if TAB_ACTIVATION_ENABLED then
    return OnTabActivation(self, editor, event)
  end
